...
router.post("/", authMiddleware(), async (req, res) => {
  try {
    ...
    const { msisdn, iccid, phoneList } = req.body;

    if (!msisdn || !iccid || !Array.isArray(phoneList) || phoneList.length === 0) {
      return res.status(400).json({
        success: false,
        message: "msisdn, iccid, and at least one phoneList item are required",
      });
    }

    const transactionId = generateTransactionId();
    const payload = { msisdn, iccid, phoneList };

    // === Call Carrier API ===
    let carrierResponse;
    try {
      carrierResponse = await axios.post(
        `${process.env.BASE_URL}/ManagePhoneList`,
        payload,
        {
          headers: {
