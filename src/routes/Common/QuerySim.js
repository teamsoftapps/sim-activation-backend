  ...
  try {
    ...
    try {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: "Failed to query ICCID at carrier",
        transactionId,
    ...
    if (hasError || result.status === "ERROR" || result.status === "FAILED") {
      return res.status(400).json({
        success: false,
        message: "ICCID query failed at carrier",
        transactionId,
    ...
    res.json({
      success: true,
      message: "ICCID info fetched successfully",
      transactionId,
      queriedBy: {
      ...
      data: {
        ...
        iccid: result.iccid,
        iccidInfo: result.iccidInfo || {},
        result: result.result || [],
      },
      apiResponse: carrierResponse.data,
    });
  } catch (err) {
    console.error("QueryICCID Internal Error:", err);
    res.status(500).json({
      success: false,
