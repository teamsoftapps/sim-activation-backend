...
router.post('/', authMiddleware(), async (req, res) => {
  try {
    ...
    const { userId, iccid, portIn, portInRequestId } = req.body;
    ...
    if (!portInRequestId) {
      return res.status(400).json({ success: false, message: 'portInRequestId is required' });
    }
    ...
    // === Include portInRequestId in payload ===
    const payload = {
      iccid,
      portIn: { portInMsisdn: portIn.portInMsisdn },
      portInRequestId,
    ...
    await PortInCancellation.create({
      ...
      msisdn: result.msisdn || '',
      status: result.status || 'SUCCESS',
      portInRequestId: result.portInRequestId || portInRequestId,
      portInDueDate: result.portInDueDate ? new Date(result.portInDueDate) : null,
      result: result.result || [],
      error: null,
      transactionId,
      cancelledAt: new Date(),
