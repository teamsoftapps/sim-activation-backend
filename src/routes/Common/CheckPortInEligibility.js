...
router.post('/', authMiddleware(), async (req, res) => {
  try {
    ...
    const targetUser = isAdmin && userId ? { _id: userId } : authenticatedEntity;
    ...
    try {
      carrierResponse = await axios.post(
        `${process.env.BASE_URL}/CheckPortInEligibility`,
        payload,
        {
          headers: {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: 'Failed to check port-in eligibility at carrier',
        transactionId,
    ...
    if (hasError) {
      return res.status(400).json({
        success: false,
        message: 'Carrier returned an error while checking eligibility',
        transactionId,
    ...
    console.error('CheckPortInEligibility Internal Error:', err);
