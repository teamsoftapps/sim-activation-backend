...
//       console.log("Received File",rows)
//       if (rows.length < 2)
...
//   data?.status === 'ERROR' ||
//   data?.status === 'FAILED' ||
//   data?.error ||                      // top-level error
//   data?.data?.error ||                // ✅ nested carrier error
//   (Array.isArray(data?.result) &&
//     data.result.some((r) => r.status === 'ERROR'));
...
//   const errorMessage =
//     data?.error?.message ||
//     data?.data?.error?.message ||     // ✅ THIS fixes SIM not found
  ...
  async (req, res) => {
    try {
      ...
      for (const row of dataRows) {
        const activationData = {};
        row.forEach((value, i) => {
  activationData[headers[i].toLowerCase()] = (value || '').toString().trim();
        ...
        try {
          ...
          console.log('Payload going to carrier:', JSON.stringify(payload, null, 2));
      ...
      // If all rows failed, send JSON response with per-row errors
      if (successes === 0) {
        return res.status(400).json({
          success: false,
          message: 'All rows failed',
          results, // detailed per-row errors
        });
      }
