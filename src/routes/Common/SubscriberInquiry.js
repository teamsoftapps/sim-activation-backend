...
router.post("/", authMiddleware(), async (req, res) => {
  try {
    ...
    const { msisdn, iccid, imsi } = req.body;

    if (!msisdn || !iccid || !imsi) {
      return res.status(400).json({
        success: false,
        message: "MSISDN, ICCID, and IMSI are required",
      });
    }

    const transactionId = generateTransactionId();
    const payload = { msisdn, iccid, imsi };

    // === Call Carrier API ===
    let carrierResponse;
    try {
      carrierResponse = await axios.post(
        `${process.env.BASE_URL}/SubscriberInquiry`,
        payload,
        {
          headers: {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: "Failed to query subscriber at carrier",
        transactionId,
    ...
    if (hasError || result.status === "ERROR" || result.status === "FAILED") {
      return res.status(400).json({
        success: false,
        message: "Subscriber inquiry failed at carrier",
        transactionId,
    ...
    res.json({
      success: true,
      message: "Subscriber inquiry successful",
      transactionId,
      queriedBy: {
    ...
    console.error("SubscriberInquiry Internal Error:", err);
    res.status(500).json({
      success: false,
