...
router.post("/", authMiddleware(), async (req, res) => {
  try {
    ...
    const { msisdn, sim, newPlan_soc } = req.body;

    if (!msisdn || !sim || !newPlan_soc) {
      return res.status(400).json({
        success: false,
        message: "msisdn, sim, and newPlan_soc are required",
      });
    }

    const transactionId = generateTransactionId();
    const payload = { msisdn, sim, newPlan_soc };
    ...
    try {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: "Failed to query products at carrier",
        transactionId,
