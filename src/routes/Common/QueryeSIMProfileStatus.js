...
router.post('/', authMiddleware(), async (req, res) => {
  try {
    ...
    const { iccid, msisdn, eUiccid } = req.body;

    // 🔐 Validation
    if (!iccid && !msisdn && !eUiccid) {
      return res.status(400).json({
        success: false,
        message: 'At least one of iccid, msisdn, or eUiccid is required',
      });
    }
    ...
    try {
      carrierResponse = await axios.post(
        `${process.env.BASE_URL}/QueryeSIMProfileStatus`,
        payload,
        {
          headers: {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: 'Failed to query eSIM profile status at carrier',
        transactionId,
    ...
    if (hasError || result.status === 'ERROR' || result.status === 'FAILED') {
      return res.status(400).json({
        success: false,
        message: 'eSIM profile status query failed at carrier',
        transactionId,
    ...
    res.json({
      success: true,
      message: 'eSIM profile status fetched successfully',
      transactionId,
      queriedBy: {
      ...
      data: {
        ...
        profileStateChangedDate: result.profileStateChangedDate,
    ...
    console.error('QueryeSIMProfileStatus Internal Error:', err);
    res.status(500).json({
      success: false,
