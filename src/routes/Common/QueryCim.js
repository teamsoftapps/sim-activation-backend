...
router.post("/", authMiddleware(), async (req, res) => {
  try {
    ...
    const { pierTicketNumber, marketsAffected, pierStatus, startTime, endTime } = req.body;

    if (
      pierTicketNumber === undefined ||
      !marketsAffected ||
      !pierStatus ||
      !startTime ||
      !endTime
      ...
      return res.status(400).json({
        success: false,
        message: "All fields (pierTicketNumber, marketsAffected, pierStatus, startTime, endTime) are required",
      });
    }

    const transactionId = generateTransactionId();
    const payload = { pierTicketNumber, marketsAffected, pierStatus, startTime, endTime };

    // === Call Carrier API ===
    let carrierResponse;
    try {
      carrierResponse = await axios.post(
        `${process.env.BASE_URL}/QueryCIM`,
        payload,
        {
          headers: {
      ...
      return res.status(axiosErr.response?.status || 500).json({
        success: false,
        message: "Failed to query CIM at carrier",
        transactionId,
    ...
    if (hasError || result.status === "ERROR" || result.status === "FAILED") {
      return res.status(400).json({
        success: false,
        message: "CIM query failed at carrier",
        transactionId,
    ...
    res.json({
      ...
      cimNotifications: result.cimNotifications || [],
      result: result.result || [],
      apiResponse: carrierResponse.data,
    });
  } catch (err) {
    console.error("QueryCIM Internal Error:", err);
    res.status(500).json({
      success: false,
